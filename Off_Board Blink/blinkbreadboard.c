 /* Rowan University
 * File: blinkbreadboard.c
 * Author: Jan Garcia
 * Course: Intro to Embedded Systems
 * Section: 1
 * Creation Data: 9/15/18
 * Lab 1 - BlinkBreadboard using the MSP430G2553
 *
 * Description: The purpose of this lab is to take the MSP43030G2553 and mount the
 * chip onto a breadboard with a bulk capacitor, 2-LEDs (red and green), a bypass capacitor,
 * and use the 3.3 V and GND from the development as supply. The microcontroller will light
 * both LEDs  at different rates and will be delay using cycles to see the pattern.
 * The LEDs are pin P1.0 and P1.6. The watchdog will be disable to not allow an automatic
 * reset and disrupt the functions of the LEDs. P1DIR0 and P1DIR6 will be "on" to let the
 * GPIO be outputs.
 *
 */
